...
:::note
If you are trying to import a private GitHub repository, you will first need to set up [authentication with GitHub](/usage/settings#authentication).
:::
...
You may create empty folders in the home directory using commands like `mkdir` from the [terminal](/usage/terminal "mention"). In such a case, you will need to refresh the list manually from the "**⋮**" dropdown menu.

Even though you can use `touch` or `curl` to create files, the home page will only list the folders present there.

:::note
**Note:** If you want to be able to [preview](/usage/previewing) your jekyll site, it must be at the home directory. Please use `mv` in the terminal to move at the top level if it is in a sub-directory.
:::

## Install dependencies

This step is required to prepare your new project for [previewing](/usage/previewing "mention") locally.

If you created a new project from within the app, unless the [#skip-bundle-install](/usage/settings#skip-bundle-install "mention") setting is enabled, jekyll will automatically do this for you. If not, please continue reading.
...
Dependency installation can be prone to errors, but most can be overcome. Please see the [troubleshooting](/other/troubleshooting "mention") guide for help with common issues.
