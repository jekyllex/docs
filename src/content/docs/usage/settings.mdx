...
This option can be used to launch the bootstrap installer manually, in a case where the [bootstrap goes corrupt](/other/troubleshooting#corrupt-bootstrap).
...
If enabled, whenever a file is opened in the in-built [Editor](/usage/manage#editing-content), JekyllEx tries to guess it's destination URL. This guessed URL is loaded by default in the [Preview](/usage/previewing#preview) tab.
...
If you change the jekyll's server default localhost port, you need to change this setting to be able to load the correct port in the [Preview](/usage/previewing#preview) tab.
...
These fields are required you wish to commit your changes in a project using `git` in the [Terminal](/usage/terminal).
...
This setting is required to be enabled if you want a full progress log including download percentages in the [Terminal](/usage/terminal).
...
To serve the project you might need to [perform it manually](/usage/projects#install-dependencies) later.
