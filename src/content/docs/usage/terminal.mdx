...
The terminal is a UI that opens up in a bottom sheet. The sheet binds with [the process service](https://docs.jekyllex.xyz/getting-started/prerequisites#service-to-run-commands), so you can view the history of commands run from the current session.
...
:::note
If [log trimming](https://docs.jekyllex.xyz/settings#trim-logs) is enabled, each time a process finishes executing, the logs list gets trimmed to show the last 200 lines only.
:::
