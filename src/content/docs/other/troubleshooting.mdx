...
If you see such an issue, please use the "[Fix binaries](/usage/settings#fix-binaries)" setting to launch the [bootstrap installer](/getting-started/prerequisites#initial-setup). This should re-install the tools. Rest assured that your home directory, where your projects reside, will not be tampered.
...
The [bootstrap](/getting-started/prerequisites#bootstrap) still bundles certain famous gems like eventmachine, nokogiri, google-protobuf, etc. But you can't use versions other than those bundled. Please specify the exact available versions in your Gemfile to continue.

By default, `bundle install` executes with the [`--prefer-local`](/usage/settings#prefer-local-gems) flag. This ensures bundler uses the available versions for the sub-dependencies instead of trying for the latest ones.

:::caution
If you cloned a repo which had a `Gemfile.lock` already, it may happen that the versions there do not work with JekyllEx. Please delete the file and [retry installation](/usage/projects#install-dependencies).
:::
...
You may even need to [fix the bootstrap](/usage/settings#fix-binaries).
